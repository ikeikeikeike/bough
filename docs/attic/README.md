...
- [INSTINCTS.md](INSTINCTS.md) — the v0.5-v0.8 instinct subsystem (`memory_backends:` config, `bough instinct mint/approve/promote/query/forget`). The current `bough instinct status/list/show/promote` (v0.9+, homunculus-backed) shares the `promote` name but is a different command with a different data model — see the top-level README's "Continuous learning" section instead.
