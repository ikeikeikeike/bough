...
// Compose backend for the Postgres plugin. Selected by `backend: compose`
// in .bough.yaml. Instead of bringing up its own postgres (nix or docker
...
const composeEngineKind = "postgres"
...
func (p *Provider) composeUp(ctx context.Context, req *api.UpReq) error {
	...
	if port <= 0 {
		return fmt.Errorf("postgres compose: invalid port %d (Ports=%v)", port, req.Ports)
	}
	...
	if file == "" || service == "" || target == "" {
		return fmt.Errorf("postgres compose: backend=compose requires extras "+
...
func (p *Provider) composeReadyCheck(ctx context.Context, port, timeoutSec int) (bool, error) {
	...
	return false, fmt.Errorf("postgres compose: not ready on port %d within %ds", port, timeoutSec)
}
