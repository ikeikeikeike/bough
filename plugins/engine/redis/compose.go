...
// Compose backend for the Redis plugin. Selected by `backend: compose` in
// .bough.yaml. Instead of bringing up its own redis-server (nix or docker
// SDK), bough adopts the project's existing docker-compose service: it
// allocates the per-worktree host port as usual, then renders a derived
// compose file that retargets that one service's published port and runs
// it under a per-worktree compose project name (which isolates the named
// volume). The compose file stays the source of truth for image, volume,
// healthcheck, and command. See pkg/composeutil for the mechanics.
//
...
// up, e.g. "redis"), compose.target_port (the container-internal port,
// e.g. "6379").
package redis
...
func (p *Provider) composeUp(ctx context.Context, req *api.UpReq) error {
	...
	if port <= 0 {
		return fmt.Errorf("redis compose: invalid port %d (Ports=%v)", port, req.Ports)
	}
	...
	if file == "" || service == "" || target == "" {
		return fmt.Errorf("redis compose: backend=compose requires extras "+
	...
	// readiness signal, not a naive host-TCP dial.
...
func (p *Provider) composeReadyCheck(ctx context.Context, port, timeoutSec int) (bool, error) {
	...
	return false, fmt.Errorf("redis compose: not ready on port %d within %ds", port, timeoutSec)
}
