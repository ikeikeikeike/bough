...
// Compose backend for the Elasticsearch plugin. Selected by
// `backend: compose` in .bough.yaml. Instead of bringing up its own ES
// node (nix or docker SDK), bough adopts the project's existing
// docker-compose service: it allocates the per-worktree host port as
// usual, then renders a derived compose file that retargets that one
// service's published port and runs it under a per-worktree compose
// project name (which isolates the named volume). The compose file stays
// the source of truth for image, volume, healthcheck, and command (e.g. a
// sudachi analysis-plugin install step). See pkg/composeutil.
//
...
// up, e.g. "elasticsearch"), compose.target_port (the container-internal
// HTTP port, e.g. "9200").
...
const composeEngineKind = "elasticsearch"
...
func (p *Provider) composeUp(ctx context.Context, req *api.UpReq) error {
	...
	if port <= 0 {
		return fmt.Errorf("elasticsearch compose: invalid port %d (Ports=%v)", port, req.Ports)
	}
	...
	if file == "" || service == "" || target == "" {
		return fmt.Errorf("elasticsearch compose: backend=compose requires extras "+
	...
	// readiness signal, not a naive host-TCP dial (which would go green
	// before ES has formed its cluster).
...
func (p *Provider) composeReadyCheck(ctx context.Context, port, timeoutSec int) (bool, error) {
	...
	return false, fmt.Errorf("elasticsearch compose: not ready on port %d within %ds", port, timeoutSec)
}
...
// volumes. No-op for the nix / docker backends, so Cleanup can call it
// unconditionally.
