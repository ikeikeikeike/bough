...
import (
	...
	esplugin "github.com/ikeikeikeike/bough/plugins/db/elasticsearch"
)
...
func main() {
	...
	if err != nil || !ready {
		...
		_ = p.Down(ctx, api.DownReq{Port: port, GracefulTimeoutSec: 60})
	...
	t2 := time.Now()
	log.Println("=== Down ===")
	if err := p.Down(ctx, api.DownReq{Port: port, GracefulTimeoutSec: 60}); err != nil {
